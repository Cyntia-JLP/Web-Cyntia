...
(function initSteps() {
  const stepData = [
    {
      icon: '🖥️',
      title: 'Agentes Cyntia',
      desc: 'Compatibles con Windows Server 2016+, Ubuntu 18.04+, Debian 10+, CentOS 7+ y macOS 12+. Instalación vía script en menos de 5 minutos.',
      code: `<span class="code-comment"># Instalación rápida</span><br>curl -sO https://cyntia.local/install.sh<br>sudo bash install.sh --server 10.0.0.1`
    },
    {
      icon: '📥',
      title: 'Pipeline de Datos',
      desc: 'Logstash normaliza syslog, Windows Event Log, auditd y APIs externas. Los datos se indexan en Elasticsearch para búsqueda instantánea sobre millones de eventos.',
      code: `<span class="code-comment"># cyntia-pipeline.yml</span><br>input: [syslog, winlog, auditd]<br>filter: normalize → enrich → dedupe<br>output: elasticsearch:9200`
    },
    {
      icon: '🧠',
      title: 'Motor de Correlación',
      desc: 'Análisis con más de 200 reglas MITRE ATT&CK preconfiguradas. Reduce los falsos positivos un 60% mediante correlación temporal y análisis de comportamiento.',
      code: `<span class="code-comment"># regla de ejemplo</span><br>if events.ssh_fail > 5 in 60s:<br>&nbsp;&nbsp;AND src_ip not in whitelist:<br>&nbsp;&nbsp;alert(CRITICAL, "Brute Force")`
    },
    {
      icon: '🔔',
      title: 'Alertas y Respuestas',
      desc: 'Notificaciones por email, Slack o webhook con contexto completo del incidente. Respuestas automáticas configurables: bloquear IP, aislar host, ejecutar script.',
      code: `<span class="code-comment"># respuesta automática</span><br>action: block_ip(src=192.168.1.45)<br>action: notify_slack(channel=#soc)<br>action: create_ticket(jira)`
    }
  ];
  ...
  function setStep(idx) {
    ...
    setTimeout(() => {
      svIcon.textContent  = d.icon;
      svTitle.textContent = d.title;
      svDesc.textContent  = d.desc;
      svCode.innerHTML    = d.code;
