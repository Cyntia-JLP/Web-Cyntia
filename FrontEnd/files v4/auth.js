...
// ─── CAMBIAR ENTRE LOGIN, REGISTRO Y RECUPERAR ────────
...
goLoginFromForgot?.addEventListener('click', e => {
  e.preventDefault();
  forgotPanel?.classList.add('hidden');
  loginPanel?.classList.remove('hidden');
  hideError(forgotError);
...
forgotForm?.addEventListener('submit', async e => {
  ...
  if (!email) {
    if (forgotError) forgotError.textContent = 'Introduce tu dirección de email';
    showError(forgotError);
    return;
  }

  if (!isValidEmail(email)) {
    if (forgotError) forgotError.textContent = 'El formato del email no es válido';
    showError(forgotError);
    return;
  }
  ...
  // En producción: POST a /api/forgot-password.php
  console.log('[Cyntia] Password reset request for:', email);
...
function onRegisterSuccess(user) {
  closeAuth();
  showToast(`✉️ Hemos enviado un código de verificación a ${user.email}. Revisa tu bandeja de entrada para activar tu cuenta.`, 'success');
}
